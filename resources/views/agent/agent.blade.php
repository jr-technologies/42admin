...
<div class="agents-listing-page">
    ...
    $first_page = URL('/get/active/agent').'?'.'page=1';
    ...
    $previousResult = URL('/get/active/agent').'?'.$convertPreviousToQueryString;
    ?>
    <?php
    $totalPaginationValue = intval(ceil($response['data']['agentCount'] / config('constants.Pagination')));
    $last_page = URL('/get/active/agent').'?'.'page='.$totalPaginationValue;
    $for_next_link = $_GET;
    ...
    $nextResult = URL('/get/active/agent').'?'.$convertToQueryString;
    ?>
    <ul class="pager">
        ...
        $result = URL('/get/active/agent').'?'.$queryString;
        ?>
