...
<div class="project-listing">
        <div class="layout text-right"><a href="{{URL::to('project')}}" class="btn-hollow">Add a Projects</a></div>
                ...
                <li class="t-d text-center">
                    <div class="pro-id border">
                        <label for="property-id-td" class="customCheckbox">
                            ...
                            <span class="fake-checkbox"></span>
                            <span class="fake-label">{{$project->id}}</span>
                        </label>
                    </div>
                    <div class="pro-city border"><p>{{$project->title}}</p></div>
                    <div class="pro-priority border text-left"><p>{{str_limit($project->description,20)}}</p></div>
                    <div class="pro-date border"><time>{{$project->createdAt}}</time></div>
                    <div class="pro-image border"><img src="@if(isset($project->images[0]->image)){{url('/').'/'.$project->images[0]->image}}@endif" alt="image description"></div>
                    <div class="pro-controls border">
                        <ul class="control-link">
                            {{Form::open(array('url'=>'delete/project','method'=>'POST'))}}
    ...
    $previousResult = URL('project/listing').'?'.$convertPreviousToQueryString;
    ?>
    <?php
    ...
    $nextResult = URL('project/listing').'?'.$convertToQueryString;
    ?>
    <ul class="pager">
        ...
        $result = URL('project/listing').'?'.$queryString;
        ?>
